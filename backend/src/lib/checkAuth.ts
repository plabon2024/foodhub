  ...
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      if (authHeader?.startsWith("Bearer ")) {
        ...
        if (token) {
          // JWT has exactly 3 parts; otherwise treat as session token
      ...
      if (sessionToken) {
        ...
        if (sessionExists?.user) {
          ...
          // Session lifetime checks (Edupilot pattern)
          const now = new Date();
      ...
      // 3. Access Token Verification (JWT) — can overwrite session result
      if (accessToken) {
